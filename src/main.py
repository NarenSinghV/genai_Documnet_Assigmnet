...
async def upload(file: UploadFile = File(...)):
    ...
    try:
        vectordb = ingest_file_to_vectordb(str(file_path))
    ...
    return {"id": file_id, "vectordb": True}


# -------------------------
# Simple RAG Query (NO LLM)
# -------------------------
@app.post("/query_rag")
async def query_rag(q: str = Form(...)):
    if len(q) < 3:
    ...
    # Extractive answer (safe, free, no OpenAI)
    answer = context[:1500]

    return JSONResponse({
        "query": q,
        "answer": answer,
        "sources": format_provenance(results)
    })